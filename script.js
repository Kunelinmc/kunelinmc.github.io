...
document.querySelector(".data-v-ujXllYgW").addEventListener("input", (e) => {
	// 尚未實作的搜尋功能
	if (e.target.value.trim() === "") {
		e.target.value = "";
		document.querySelector(".data-v-emr1y6ie").classList.remove("show");
	} else {
		document.querySelector(".data-v-emr1y6ie").classList.add("show");
	}
});

document.querySelector(".data-v-tsARDHTI").addEventListener("input", (e) => {
	// 防止輸入框中有不必要的元素
...
document.querySelector(".data-v-tsARDHTI").addEventListener("keydown", (e) => {
	switch (e.keyCode) {
		case 13: // 沒有shift鍵 + 不為空 + 不是手機 -> 送出(非換行)
			if (!e.shiftKey && e.target.innerHTML.trim() !== "" && !isMobileDevice()) {
				e.preventDefault();
...
document.querySelector(".data-v-uwRtDpu4").onclick = function () {
	// 按下按鈕送出
	if (document.querySelector(".data-v-tsARDHTI").innerHTML.trim() !== "") {
		if (_N0) _A16(document.querySelector(".data-v-tsARDHTI").innerText);
		else console.log("Unable to send the message");
	}
};

document.querySelector(".data-v-tsARDHTI").onpaste = function (t) {
	t.preventDefault();
...
document.onkeydown = function (e) {
	...
	switch (e.keyCode) {
		...
		case 191:
			if (
				document.activeElement !== document.querySelector(".data-v-tsARDHTI") &&
				document.activeElement !== document.querySelector(".data-v-ujXllYgW")
			) {
				e.preventDefault();
				document.querySelector(".data-v-tsARDHTI").focus();
			}
			break;
		case 123:
			// "F12": 防止用戶開啟 DevTools
...
function _A16(text) {
	// 發送前置步驟，規避HTML字符
	document.querySelector(".data-v-tsARDHTI").innerHTML = "";
	_A17(
		text
			.replaceAll("<", "&lt;")
			.replaceAll(">", "&gt;")
	...
	if (_GF === false) {
		GetData("history")
			.then((data) => {
				...
				if (document.querySelector(".data-v-nKzgeITl"))
...
const msgfr = document.querySelector(".data-v-ikyLwDf4");
function _A25(text, uid, sender) {
	...
	msgfr.appendChild(f);
	if (Math.abs((scrollTop + divHeight) - contentHeight) < 80) {
		msgfr.scrollTo(0, msgfr.scrollHeight)
	}
}
window._A25 = _A25;
